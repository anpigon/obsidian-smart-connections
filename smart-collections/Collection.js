...
class Collection {
  ...
  static load(brain, config = {}) {
    ...
    if (brain[this.collection_name].load instanceof AsyncFunction)
      return brain[this.collection_name]
        .load()
        .then(() => brain[this.collection_name]);
  ...
  merge_defaults() {
    ...
    while (current_class) {
      const col_conf =
        (_b = (_a = this.config) == null ? void 0 : _a.collections) == null
          ? void 0
          : _b[current_class.collection_name];
      Object.entries(typeof col_conf === "object" ? col_conf : {}).forEach(
  ...
  reviver(key, value) {
    ...
    if (value.class_name)
      return new this.brain.item_types[value.class_name](this.brain, value);
    return value;
  }
  ...
  filter(opts) {
    return this.keys
      .filter((key) => this.items[key].filter(opts))
