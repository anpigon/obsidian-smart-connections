const { Brain, Collection, CollectionItem } = require("smart-collections");
...
class SmartBrain extends Brain {
  ...
  async init() {
    ...
    this.smart_markdown = new SmartMarkdown({
      ...this.config,
      skip_blocks_with_headings_only: true,
    }); // initialize smart markdown (before collections, b/c collections use smart markdown)
    await Promise.all(
      Object.values(this.collections).map(
        async (static_collection) => await static_collection.load(this)
      )
    );
  ...
  async init_import() {
    if (this.smart_notes.smart_embed || this.smart_blocks.smart_embed)
      this.smart_notes.import({ reset: true, show_notice: true });
  }

  get_tfile(file_path) {
    return this.main.app.vault.getAbstractFileByPath(file_path);
  }
  ...
  save() {
    ...
    this.save_timeout = setTimeout(async () => {
      ...
      if (
        this.main.last_user_activity &&
        Date.now() - this.main.last_user_activity < 60000
      )
        return this.save(); // reset save timeout
      await this._save();
      this.save_timeout = null;
  ...
  async _save() {
    await Promise.all(
      Object.keys(this.collections).map(
        async (collection_name) => await this[collection_name]._save()
      )
    );
  }
  // getters
  get all_files() {
    return this.main.app.vault
      .getFiles()
      .filter(
        (file) =>
          file instanceof this.main.obsidian.TFile &&
    ...
    return this.main.app.vault
      ...
      .filter(
        (file) =>
          file instanceof this.main.obsidian.TFile &&
          (file.extension === "md" || file.extension === "canvas") &&
          this.is_included(file.path)
      );
  }
  is_included(file_path) {
    return !this.file_exclusions.some((exclusion) =>
      file_path.includes(exclusion)
    );
  }

  get file_exclusions() {
    if (this._file_exclusions) return this._file_exclusions;
    this._file_exclusions = this.main.settings.file_exclusions?.length
      ? this.main.settings.file_exclusions.split(",").map((file) => file.trim())
      : [];
    return (this._file_exclusions = this._file_exclusions.concat(
      this.folder_exclusions
    )); // merge file exclusions with folder exclusions (parser only checks this.file_exclusions)
  }
  get folder_exclusions() {
    if (this._folder_exclusions) return this._folder_exclusions;
    return (this._folder_exclusions = this.main.settings.folder_exclusions
      ?.length
      ? this.main.settings.folder_exclusions.split(",").map((folder) => {
          folder = folder.trim();
  ...
  get excluded_headings() {
    ...
    return (this._excluded_headings = this.main.settings.excluded_headings
      ?.length
      ? this.main.settings.excluded_headings
          .split(",")
...
class SmartEntities extends Collection {
  ...
  replacer(key, value) {
    ...
    if (value instanceof this.item_type) {
      ...
      if (value.data.embedding.vec && value.data.text)
        value.data.text = undefined; // clear text if embedding exists
      return value.data;
    }
  ...
  async load_smart_embed() {
    ...
    if (
      this.brain.smart_embed_active_models[this.smart_embed_model] instanceof
      SmartEmbed
    ) {
      this.smart_embed =
        this.brain.smart_embed_active_models[this.smart_embed_model];
      console.log(
        "SmartEmbed already loaded for " +
          this.collection_name +
          ": Model: " +
          this.smart_embed_model
      );
    } else {
      if (this.smart_embed_model.includes("/")) {
        // TODO: better way to detect local model
        if (this.brain.local_model_type === "Web") {
          while (!this.brain.main.view?.containerEl)
            await new Promise((resolve) => setTimeout(resolve, 100)); // wait for this.main.view.containerEl to be available
          ...
          this.smart_embed = await SmartEmbedTransformersWebAdapter.create(
            this.smart_embed_model,
            this.brain.main.view.containerEl,
            web_script
          ); // initialize smart embed
        } else {
          ...
          this.smart_embed = await SmartEmbedTransformersNodeAdapter.create(
            this.smart_embed_model
          ); // initialize smart embed
        }
      } else {
        ...
        this.smart_embed = await SmartEmbedOpenAIAdapter.create(
          this.smart_embed_model,
          this.brain.main.obsidian.requestUrl,
          this.config.api_key
        ); // initialize smart embed
      }
  ...
  async ensure_embeddings(show_notice = null) {
    ...
    if (this.smart_embed.is_embedding)
      return console.log(
        "already embedding, skipping ensure_embeddings",
        this.smart_embed.queue_length
      );
    const unembedded_items = Object.values(this.items).filter(
      (item) => !item.vec
    ); // gets all without vec
    ...
    const performance_notice_msg =
      "(Processing a large number of notes using the local models may cause Obsidian to become temporarily unresponsive.)";
    if (show_notice !== false && unembedded_items.length > 30) {
      const start_btn = {
        text: "Start embedding",
        callback: () => this.ensure_embeddings(false),
      };
      this.brain.main.show_notice(
        [
          `Are you ready to begin embedding ${unembedded_items.length} ${this.collection_name}?`,
          performance_notice_msg,
        ],
        { timeout: 0, button: start_btn }
      );
      return;
    }
    for (let i = 0; i < unembedded_items.length; i += batch_size) {
      if (this._pause_embeddings) {
        ...
        const restart_btn = {
          text: "Restart",
          callback: () => this.ensure_embeddings(),
        };
        this.brain.main.show_notice(
          [
            `Embedding ${this.collection_name}...`,
            `Paused at ${i} / ${unembedded_items.length} ${this.collection_name}`,
            performance_notice_msg,
          ],
          { timeout: 0, button: restart_btn }
        );
      ...
      if (i % 10 === 0) {
        const pause_btn = {
          text: "Pause",
          callback: () => this.pause_embedding(),
          stay_open: true,
        };
        this.brain.main.show_notice(
          [
            `Embedding ${this.collection_name}...`,
            `Progress: ${i} / ${unembedded_items.length} ${this.collection_name}`,
            performance_notice_msg,
          ],
          { timeout: 0, button: pause_btn }
        );
      }
    ...
    if (this.brain.main._notice?.noticeEl?.parentElement)
      this.brain.main._notice.hide();
    this.brain.main.show_notice(
      [
        `Embedding ${this.collection_name}...`,
        `Done creating ${unembedded_items.length} embeddings.`,
      ],
      { timeout: 10000 }
    );
  ...
  nearest(vec, filter = {}) {
    ...
    const { results_count = 20 } = filter;
    // const timestamp = Date.now();
    const nearest = this.filter(filter).reduce(
      (acc, item) => {
...
class SmartEntity extends CollectionItem {
  ...
  filter(filter = {}) {
    const { include_path_begins_with = null } = filter;
    ...
    if (include_path_begins_with) {
      const paths = Array.isArray(include_path_begins_with)
        ? include_path_begins_with
        : [include_path_begins_with];
  ...
  get name() {
    return (
      !this.brain.main.settings.show_full_path
        ? this.path.split("/").pop()
...
class SmartNotes extends SmartEntities {
  async import(opts = {}) {
    const { file_path = null, reset = false, show_notice = false } = opts;
    // if(reset) this.clear();
    if (file_path) {
      ...
      if (this.brain.smart_blocks.smart_embed)
        await this.brain.smart_blocks.smart_embed.embed_batch(
          smart_note.blocks
        );
      return;
    }
    ...
    try {
      const files = [];
      this.brain.files.forEach((file) => {
        if (!this.get(file.path)) files.push(file);
      }); // get files that aren't already imported
      ...
      await Promise.all(batch);
      if (this.brain.main._notice_content?.textContent)
        this.brain.main._notice_content.textContent =
          "Making Smart Connections... Done importing Smart Notes.";
      if (files.length) {
        await this._save();
  ...
  prune(override = false) {
    const remove = [];
    const items_w_vec = Object.entries(this.items).filter(
      ([key, note]) => note.vec
    );
    ...
    if (
      (override && remove_ratio < 0.5) ||
      confirm(
        `Are you sure you want to delete ${remove.length} (${Math.floor(
          remove_ratio * 100
  ...
  get current_note() {
    return this.get(this.brain.main.app.workspace.getActiveFile().path);
  }
}
class SmartNote extends SmartEntity {
  ...
  update_data(data) {
    if (!this.is_new && this.last_history) {
      ...
      if (
        last.mtime === this.t_file.stat.mtime ||
        last.size === this.t_file.stat.size
      )
        return false;
    }
    ...
    this.data.history.push({
      blocks: [],
      mtime: this.t_file.stat.mtime,
      size: this.t_file.stat.size,
    }); // add history entry
    return true;
  }
  async get_content() {
  ...
  async init() {
    ...
    const { blocks } = this.brain.smart_markdown.parse({
      content,
      file_path: this.data.path,
    }); // create blocks from note content
    blocks.forEach((block, i) => {
      if (
        this.brain.excluded_headings.some((exclusion) =>
          block.path.includes(exclusion)
        )
      )
        return; // skip excluded headings
  ...
  find_connections() {
    ...
    if (!this.vec && !this.median_block_vec) {
      ...
      this.brain.main.show_notice(`No embeddings found for ${this.name}.`, {
        button: start_embedding_btn,
      });
      return results;
    }
    if (
      this.vec &&
      this.median_block_vec &&
      this.brain.smart_blocks.smart_embed &&
      this.collection.smart_embed
    ) {
      const nearest_blocks = this.brain.smart_blocks.nearest(
        this.median_block_vec,
        { exclude_key_starts_with: this.key }
      );
      const nearest_notes = this.brain.smart_notes.nearest(this.vec, {
        exclude_key_starts_with: this.key,
      });
      results = nearest_blocks
        .map((block) => {
          const note = nearest_notes.find(
            (note) => note.key === block.note_key
          );
      ...
      results = nearest_blocks
        .map((block) => {
          ...
          block.score =
            (block.sim +
              cos_sim(this.median_block_vec, block.note.median_block_vec)) /
            2;
          return block;
        })
        // sort by item.score descending
  ...
  open() {
    this.brain.main.open_note(this.data.path);
  }
  get_block_by_line(line) {
    return this.blocks.find(
      (block) => block.data.lines[0] <= line && block.data.lines[1] >= line
    );
  }
  get block_vecs() {
    return this.blocks
      .map((block) => block.data.embedding.vec)
      .filter((vec) => vec);
  } // filter out blocks without vec
  get blocks() {
    return this.last_history.blocks
      .map((block_key) => this.brain.smart_blocks.get(block_key))
      .filter((block) => block);
  ...
  get mean_block_vec() {
    return this._mean_block_vec
      ...
      : (this._mean_block_vec = this.block_vecs
          .reduce(
  ...
  get median_block_vec() {
    return this._median_block_vec
      ? this._median_block_vec
      : (this._median_block_vec = this.block_vecs[0]?.map(
          (val, i) =>
            this.block_vecs.map((vec) => vec[i]).sort()[
              Math.floor(this.block_vecs.length / 2)
            ]
        ));
  }
  get t_file() {
    return this.brain.get_tfile(this.data.path);
  }
}
class SmartBlocks extends SmartEntities {
  ...
  prune(override = false) {
    const remove = [];
    const total_items_w_vec = Object.entries(this.items).filter(
      ([key, block]) => block.vec
    ).length;
...
function top_acc(_acc, item, ct = 10) {
  if (_acc.items.size < ct) {
    ...
    _acc.minItem = Array.from(_acc.items).reduce((min, curr) =>
      curr.sim < min.sim ? curr : min
    );
...
class SmartBlock extends SmartEntity {
  ...
  async get_content() {
    ...
    const block_content = this.brain.smart_markdown.get_block_from_path(
      this.data.path,
      note_content
    );
    return block_content;
  }
  async get_as_context_for_chat() {
    return this.breadcrumbs + "\n" + (await this.get_content());
  }
  find_connections() {
    if (!this.vec) return [];
    return this.brain.smart_blocks.nearest(this.vec, {
      exclude_key_starts_with: this.note.key,
    });
  }
  get breadcrumbs() {
    return this.data.path
  ...
  get is_gone() {
    return (
      !this.note ||
      this.note.is_gone ||
      !this.note.last_history.blocks.includes(this.key)
    );
  }
  ...
  get name() {
    return (
      !this.brain.main.settings.show_full_path
        ? this.data.path.split("/").pop()
        : this.data.path.split("/").join(" > ")
    )
      .split("#")
      .join(" > ")
      .replace(".md", "");
  ...
  get note() {
    return this.brain.smart_notes.get(this.note_key);
  }
  get note_key() {
